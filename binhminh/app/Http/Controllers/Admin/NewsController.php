...
class NewsController extends Controller {
	...
	public function store( Request $request)
	{
		...
		$new_img = $request->input('add_images_create_news');
		$new_img = "[".$new_img."]";
		$new->image_list = $new_img;
...
//exit();
		$new->the_tu_khoa=$request->input('the_tu_khoa');
		$new->the_gioi_thieu=$request->input('the_gioi_thieu');
		$new_img = $request->input('image_news_avt');
		$new_img = "[".$new_img."]";
		$new->image_list = $new_img;
	...
	public function loc_san_pham($tac_gia,$cat11 ) {
		...
		return view('admin.news.index')->with('news',$news)->with('session_value',$session_value)->with('all_news',$all_news)->with('all_news_0',$all_news_0)->with('all_news_1',$all_news_1)->with('s_search',$s_search)->with('catnews',$catnews)->with('catnew_parents',$catnew_parents)->with('cat11',$cat11)->with('admin_changes',$admin_changes)->with('tac_gia',$tac_gia)->with('cat_change',$cat_changes);
